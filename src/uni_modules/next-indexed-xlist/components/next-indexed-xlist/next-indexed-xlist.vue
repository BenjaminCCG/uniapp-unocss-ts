<template>
  <view class="next-list">
    ...
    >
      ...
      >
        <view :id="`item${index}`" class="left-item-title" v-if="item && item.length">
          {{ index }}
        </view>
        <view
        ...
        >
          ...
          >
            ...
            <view class="left-item-card-phone" v-else-if="mess[phoneKey]">
              {{ mess[phoneKey] }}
            </view>
          </view>
        </view>
...
import pinyin from './js-pinyin/js/pinyin.js'
      ...
      if (this.searchStr) {
        ...
        for (let i in this.scrollLeftObj) {
          this.scrollLeftObj[i] = this.scrollLeftObj[i].filter((item) => {
            return (
              item[this.nameKey].indexOf(this.searchStr) != -1 ||
              item[this.phoneKey].indexOf(this.searchStr) != -1
            )
          })
